...
> Доделать тесты которые будут сами запускать docker и потом тестировать. + более детально покрыть, ++ Запаковать етстирование в докер

> Улучшить обработку ошибок, и содать формат API

> Сейчас на каждый запрос к бирже создается новая сессия. Это плохо сказывается на количестве одновременных запросов к бирже. Надо делать новую сессию на каждые N запросов
